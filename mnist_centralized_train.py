...
train_loader = torch.utils.data.DataLoader(train_set, batch_size=128, shuffle=True, num_workers=6, persistent_workers=True)

test_set = torchvision.datasets.MNIST('files', train=False, download=True, transform=torchvision.transforms.Compose(
    [torchvision.transforms.Resize((224, 224)),
     torchvision.transforms.ToTensor(), torchvision.transforms.Normalize((0.1307,), (0.3081,))]))
test_loader = torch.utils.data.DataLoader(train_set, batch_size=1024, shuffle=True, num_workers=6, persistent_workers=True)
...
gym = Gym(train_loader=train_loader, val_loader=test_loader, model=model, criterion=criterion, optimizer=optimizer,
          device="cuda", verbose=True, metric=sklearn.metrics.balanced_accuracy_score)
gym.train(100, 100)

print("hic featurecloud est")
